...
class waimaiProcessor(DataProcessor):
  ...
  def get_train_examples(self, data_dir):
    ...
    train_data=[]
    index=0
    for index,train in enumerate(f.values):
        guid = 'train-%d' % index
        #line = line.replace("\n", "").split("\t")
        text_a = tokenization.convert_to_unicode(str(train[1]))
        text_b = tokenization.convert_to_unicode(str(train[0]))
        label=str(train[0])
  ...
  def get_dev_examples(self, data_dir):
    ...
    dev_data=[]
    index=0
    for index,dev in enumerate(f.values):
        guid = 'dev-%d' % index
        #line = line.replace("\n", "").split("\t")
        text_a = tokenization.convert_to_unicode(str(dev[1]))
        text_b = tokenization.convert_to_unicode(str(dev[0]))
        label=str(dev[0])
  ...
  def get_test_examples(self, data_dir):
    ...
    f=pd.read_csv(file_path,encoding='utf-8')
    test_data=[]
    for index,test in f.values:
        guid = 'test-%d' % index
        #line = line.replace("\n", "").split("\t")
        text_a = tokenization.convert_to_unicode(str(test[1]))
        text_b = tokenization.convert_to_unicode(str(test[0]))
        label=str(test[0])
